...
new g_levelprowess, g_levelprotection, g_levelswiftness, g_levelmarksmanship, g_levelhealth

new const LEVEL_ENHANCE[10] = 
{
    1,		
	2,		
	3,		
...
public CPlayer__TakeDamage_Post(victim, inflictor, attacker, Float:damage)
{
	...
	if(is_user_valid_connected(inflictor) && get_user_weapon(inflictor) == CSW_KNIFE && !g_zombie[attacker] && g_zombie[victim])
	{
		...
		new Float:knockbackprowess = g_levelprowess[attacker]
	
		if(g_knife[attacker] == strong)
		{
			velocity[0] = newvelocity[0] * (1.1 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
			velocity[1] = newvelocity[1] * (1.1 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
		}
		else if(g_knife[attacker] == axe)
		{
			velocity[0] = newvelocity[0] * (0.7 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
			velocity[1] = newvelocity[1] * (0.7 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
		}
		else if(g_knife[attacker] == combat)
		{
			velocity[0] = newvelocity[0] * (0.4 + knockbackprowess)  * 3000 / get_distance_f(victim_origin, attacker_origin)
			velocity[1] = newvelocity[1] * (0.4 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
		}
		else if(g_knife[attacker] == katana)
		{
			velocity[0] = newvelocity[0] * (1.3 + knockbackprowess)  * 3000 / get_distance_f(victim_origin, attacker_origin)
			velocity[1] = newvelocity[1] * (1.3 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
		}
		else if(g_knife[attacker] == hammer)
		{
			if(!g_mode[attacker])
			{
				velocity[0] = newvelocity[0] * (12.3 + knockbackprowess)  * 3000 / get_distance_f(victim_origin, attacker_origin)
				velocity[1] = newvelocity[1] * (12.3 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
			}
			else
			{
				velocity[0] = newvelocity[0] * (25.0 + knockbackprowess)  * 3000 / get_distance_f(victim_origin, attacker_origin)
				velocity[1] = newvelocity[1] * (25.0 + knockbackprowess) * 3000 / get_distance_f(victim_origin, attacker_origin)
			}
		}
...
public show_enhance_menu(id) 
{
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yPlayer Enchancement \r|| \wLevels \r: \y%d\r/\y130 \r- \y%d\r/\y%d \wXP^n^n", g_iLevel[id], g_iExp[id], MAX_LEVELS[g_iLevel[ id ]]
	// Items
	len += formatex(menu[len], charsmax(menu) - len, "\r[HB/ZB]\w Physical Prowess : \r%d/10^n", g_levelprowess[LEVEL_ENHANCE[id]])
	len += formatex(menu[len], charsmax(menu) - len, "\r[HB/ZB]\w Physical Protection : \r%d/10^n", g_levelprotection[LEVEL_ENHANCE[id]])
	len += formatex(menu[len], charsmax(menu) - len, "\r[HB]\w Marksmanship : \r%d/10^n", g_levelmarksmanship[LEVEL_ENHANCE[id]])
	len += formatex(menu[len], charsmax(menu) - len, "\r[ZB]\w Bonus Health : \r%d/10^n^n", g_levelhealth[LEVEL_ENHANCE[id]])
	len += formatex(menu[len], charsmax(menu) - len, "\r1\w \y[\wINCREASE STATS\y]^n", g_levelhealth[LEVEL_ENHANCE[id]])
...
public show_game_menu(id) 
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "\r3.\w Player Enchancement^n")
