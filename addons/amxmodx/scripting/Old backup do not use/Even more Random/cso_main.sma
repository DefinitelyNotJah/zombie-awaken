...
new g_iMaxClients, g_MsgTeamInfo, g_msgDeathMsg, g_msgBarTime, g_msgScreenShake, g_msgScreenFade, gmsgWeaponList, g_msgScoreInfo,
g_msgScoreAttrib, g_msgAmmoPickup

new g_save, g_save2, g_save3, g_save4
new g_roundstart, g_freeround
...
new mdl_light, mdl_psy, mdl_china, mdl_shaman, mdl_deimos
new mdl_males, mdl_females
...
new g_iLevel[ MAX_CLIENTS + 1 ], g_iExp[ MAX_CLIENTS + 1 ]
...
new g_unlocked[33] // unlock Abilitys
...
public plugin_init() {
	...
	register_clcmd("admin_set_user_skin", "clcmd_set_user_skin", ADMIN_RCON, "- admin_set_user_skin <name> <number>")
	register_clcmd("admin_show_skin", "clcmd_show_skin", ADMIN_RCON)
...
public clcmd_show_skin(id, level, cid)
{
	if ( !cmd_access ( id, level, cid, 3 ) )
		return PLUGIN_HANDLED;
		
	client_print(id, print_console, "===================================")
	client_print(id, print_console, "admin_set_user_skin <player> <skin>")
	client_print(id, print_console, "admin_set_user_skin <player> <skin>")
	client_print(id, print_console, "0. Normal Human Models")
	client_print(id, print_console, "1. Normal Human Models")
	client_print(id, print_console, "2. Blair")
	client_print(id, print_console, "3. Guns Smith")
	client_print(id, print_console, "4. Natasha")
	client_print(id, print_console, "5. Pirate Girl")
	client_print(id, print_console, "6. Marine Girl")
	client_print(id, print_console, "7. Alice")
	client_print(id, print_console, "8. Marine Boy")
	client_print(id, print_console, "9. General")
	client_print(id, print_console, "10. Pirate Boy")
	client_print(id, print_console, "===================================")
	return PLUGIN_HANDLED;
}
...
public logevent_round_end()
{
	...
	else
	{
		...
		client_print(0, print_center, "Out of World!")
	}
	balance_teams()
}
...
public fw_ClientDisconnect(id) {
	...
	if(g_zombie[id])
	{
		if(!fnGetZombies() && iPlayersnum >= 2)
		{
			...
			infect_user(newid)
			client_print(0, print_chat, "the Zombie %s Leave, the new Zombie is %s", szName, szNewName)
		}
	}
	else
	{
...
public new_round_count()
{
	...
	else
	{
		...
		{
				tempid = players [ i ]
				set_pev(tempid, pev_gravity, 1.0)
				client_print(tempid, print_chat, "Time to start the game")
		}
	}
}
...
public make_user_hero(id)
{
	...
	switch(Random)
	{
		case 0 :
		{
			...
			client_print(0, print_chat, "%s has choosed as HERO!", name)
		}
		case 1 :
		{
			cs_set_player_model(id, "legendofwarior_females")
			...
			client_print(0, print_chat, "%s has choosed as HEROINE!", name)
		}
	}
}
public round_song()
...
public ShowHUD(taskid)
{
	...
	if(g_roundstart)
	{
		...
		show_dhudmessage(id, "Health | %d", get_user_health(id))
	}
	else
	{
		set_dhudmessage(255, 0, 0, 0.02, 0.9, 0, 6.0, 1.0)
		show_dhudmessage(id, "Health | %d || Levels: %d/130 || XP: %d/%d", get_user_health(id), g_iLevel[id], g_iExp[id], MAX_LEVELS[g_iLevel[ id ]])
	}
...
public show_game_menu(id) {
	...
	// Title
	len += formatex(menu[len], charsmax(menu) - len, "\yGame Menu \r|| \wLevels \r: \y%d\r/\y130 \r- \y%d\r/\y%d \wXP^n^n", g_iLevel[id], g_iExp[id], MAX_LEVELS[g_iLevel[ id ]])
	// Items
	len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Choose Zombie Class^n")
	len += formatex(menu[len], charsmax(menu) - len, "\r2.\w Registration Zone^n")
...
stock set_player_nextattack(id, Float:nexttime)
{
	...
	set_pdata_float(id, m_flNextAttack, nexttime, PLAYER_LINUX_XTRA_OFF)
}
...
public Save_Data(id)
{
	new vaultkey[64], vaultdata[256], vaultdata2[256], vaultdata3[256], vaultdata4[256]
...
public Load_Data(id)
{
	...
	new XP[32], Level[32], Knifas[32], a[32]
