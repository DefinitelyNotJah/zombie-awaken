...
g_msgHideWeapon, g_msgCrosshair, Float:g_flLastNvgToggle[33], bool:g_bNvgOn[33], g_szLightStyle[2], 
g_pCvarNVisionDensity,g_pCvarZombieNVisionColors[3], g_pCvarLightingStyle, g_fwRoundStart, g_fwDummyResult,
g_damagedealt[33], g_msgBarTime
...
public plugin_init() {
	...
	register_clcmd("cso_give_crystal", "clcmd_set_user_crystal", ADMIN_RCON, "<name> <amount>")
...
public clcmd_get_user_stats(id)
{
	...
	for (Iterator = 1; Iterator <= get_maxplayers(); Iterator++)
	{
		if (is_user_connected(Iterator))
		{
			...
			{
				client_print ( id, print_console, "[ADMIN-CSO] Player %s / Level %d / EXP %d / Crystals %d / P-D-S-M-H %d-%d-%d-%d-%d", s_Name, g_iLevel[Iterator], g_iExp[Iterator], g_iCrystal[Iterator], g_levelprowess[Iterator], g_levelprotection[Iterator], g_levelswiftness[Iterator], g_levelmarksmanship[Iterator], g_levelhealth[Iterator]);
			}
		}
	}
...
public clcmd_add_user_level(id, level, cid)
{
	...
	if(str_to_num(s_Amount) + g_iLevel[i_Target] < 100)
	{
		...
		client_printc(i_Target, "!g[CSO]!n %s gave you %s Crystal(s).", s_Admin, s_Amount)
		Save_Data(i_Target)
	}
	else
	{
...
public clcmd_set_user_crystal(id, level, cid)
{
	...
	if(str_to_num(s_Amount) + g_iCrystal[i_Target] < 200)
	{
		g_iCrystal[i_Target] = str_to_num(s_Amount) + g_iCrystal[i_Target]
		client_print ( id, print_console, " [ADMIN-CSO] You gave %s %s crystals", s_Name, s_Amount)
		client_printc(i_Target, "!g[CSO]!n %s gave you %s Crystal(s).", s_Admin, s_Amount)
		Save_Data(i_Target)
	}
	else
...
public plugin_precache()
{
	...
	precache_sound("zm_cso/roundstart_special.mp3")
...
public message_textmsg()
{
	...
	elseif (equal(textmsg, "#Game_Commencing"))
	{
		g_roundstart = false
	} 
...
public OnTouchCrystal(ent, id)
{
	...
	{
		remove_entity(ent)
		client_printc(id, "!g[CSO]!n You picked up a Crystal!")
		client_printc(id, "!g[CSO]!n Say !t/shop !nand spend your crystals.")
		g_iCrystal[id]++
...
public event_round_start()
{
	...
	if(round_count > 1)
	{
		...
		set_task(12.0, "play_countdown", 5810)
		g_freeround = false
	}
	else
	{
...
public show_shop_menu(id) {
	...
	// Items
	if(g_iCrystal[ id ] >= 1)
		len += formatex(menu[len], charsmax(menu) - len, "\r1.\w Ammunition \y[1 Crystal]^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r1.\d Ammunition \y[1 Crystal]^n")
	if(g_iCrystal[ id ] >= 5)
		len += formatex(menu[len], charsmax(menu) - len, "\r2.\w M67 Grenade \y[5 Crystals]^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r2.\d M67 Grenade \y[5 Crystals]^n")
	if(g_iCrystal[ id ] >= 7)
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\w Holy Water Bottle \y[7 Crystals]^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r3.\d Holy Water Bottle \y[7 Crystals]^n")
	if(g_iCrystal[ id ] >= 15)
...
public show_upgrade_menu(id) 
{
	...
	if(g_levelpoints[id] >= 2 && g_levelprowess[id] < 10)
		len += formatex(menu[len], charsmax(menu) - len, "\r1. [HB/ZB]\w Physical Prowess\y++^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r1. [HB/ZB]\d Physical Prowess^n")
	if(g_levelpoints[id] >= 2 && g_levelprotection[id] < 10)
		len += formatex(menu[len], charsmax(menu) - len, "\r2. [HB/ZB]\w Physical Protection\y++^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r2. [HB/ZB]\d Physical Protection^n")
	if(g_levelpoints[id] >= 2 && g_levelswiftness[id] < 10)
		len += formatex(menu[len], charsmax(menu) - len, "\r3. [HB/ZB]\w Physical Swiftness\y++^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r3. [HB/ZB]\d Physical Swiftness^n")
	if(g_levelpoints[id] >= 2 && g_levelmarksmanship[id] < 10)
		len += formatex(menu[len], charsmax(menu) - len, "\r4. [HB/ZB]\w Marksmanship\y++^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r4. [HB/ZB]\d Marksmanship^n")
	if(g_levelpoints[id] >= 2 && g_levelhealth[id] < 10)
		len += formatex(menu[len], charsmax(menu) - len, "\r5. [HB/ZB]\w Bonus Health\y++^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r5. [HB/ZB]\d Bonus Health^n")
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^n\r9. \wGo Back^n")
	len += formatex(menu[len], charsmax(menu) - len, "\r0.\y Exit")
...
public show_enhance_menu(id) 
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "^n^n^n\r9. \y[\w---INCREASE STATS---\y]^n")
	len += formatex(menu[len], charsmax(menu) - len, "\r0.\y Exit")
...
public show_game_menu(id) 
{
	...
	if(g_zombie[id])
		len += formatex(menu[len], charsmax(menu) - len, "\r2.\d Mysterious Store^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r2.\w Mysterious Store^n")
	len += formatex(menu[len], charsmax(menu) - len, "\r3.\w Player Enhancement^n")
	if(get_user_flags(id) & ADMIN_KICK)
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Join Spectator^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\d Join Spectator^n")
...
public task_hide_money(taskid)
{
	...
	// Hide money
	message_begin(MSG_ONE, g_msgHideWeapon, _, taskid)
	write_byte(HIDE_MONEY) // what to hide bitsum
	message_end()

	// Hide the HL crosshair that's drawn
	message_begin(MSG_ONE, g_msgCrosshair, _, taskid)
	write_byte(0) // toggle
	message_end()
}
