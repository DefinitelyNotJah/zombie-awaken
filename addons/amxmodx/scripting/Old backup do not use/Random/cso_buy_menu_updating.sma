...
public show_buy_menu(id) 
{
	...
	if(g_primary[id]){
		...
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r3\y].\d Primary Assault Rifles^n")
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r4\y].\d Primary Sniper Rifles^n")
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r5\y].\d Primary Machine Guns^n^n")
	}
	else
	{
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r2\y].\w Primary ShotGuns^n")
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r3\y].\w Primary Assault Rifles^n")
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r4\y].\w Primary Sniper Rifles^n")
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r5\y].\w Primary Machine Guns^n^n")
	}
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r6\y].\w Choose a Knife^n")
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r7\y].\w Equipment")
...
public eq_m(id)
{
	...
	if(cso_get_user_money(id) < 1200)
		formatex(string, sizeof(string), "\dHoly Water Bottle - \y[\r1200$\y]")
	else
		formatex(string, sizeof(string), "\wHoly Water Bottle \d- \y[\r1200$\y]")
	menu_additem(menu, string, "2", 0)
	
	if(cso_get_user_money(id) < 5000 || g_boughtarmor[id])
		formatex(string, sizeof(string), "\dAnti-Infection Armor (50)- \y[\r5000$\y] \r(max : 1)")
	else
		formatex(string, sizeof(string), "\wAnti-Infection Armor (50)\d- \y[\r5000$\y] \r(max : 1)")
	menu_additem(menu, string, "3", 0)

	if(get_user_flags(id) & ADMIN_LEVEL_H)
	{
		if(cso_get_user_money(id) < 7500 || g_boughtarmor[id])
			formatex(string, sizeof(string), "\dAnti-Infection Armor (100)- \y[\r7500$\y][\rVIP\y] \r(max : 1)")
		else
			formatex(string, sizeof(string), "\wAnti-Infection Armor (100)\d- \y[\r7500$\y][\rVIP\y] \r(max : 1)")
	}
	else
		formatex(string, sizeof(string), "\wAnti-Infection Armor (100)\d- \y[\r7500$\y][\rVIP\y] \r(max : 1)")
	menu_additem(menu, string, "4", 0)
