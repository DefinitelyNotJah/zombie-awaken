...
//MySQL
new g_pCvarDBInfo[4], Handle:g_hTuple
new const g_szTable[] =
" CREATE TABLE IF NOT EXISTS my_table ( `Level` int(20) default NULL, `XP` int(20) default NULL, `KNIFE` int(20) default NULL, `SKIN` int(20) default NULL, `MONEY` int(20) default NULL, `NAME` varchar(64) default NULL, `SteamID` varchar(64) NOT NULL, PRIMARY KEY(`STEAMID`));"
...
Float:g_flLastNvgToggle[33], bool:g_bNvgOn[33], g_szLightStyle[2], g_pCvarLightingStyle, g_fwRoundStart, g_fwDummyResult,
g_damagedealt[33], g_msgBarTime, g_MsgMoney, g_msgMoneyBlink, g_fwUserInfected, g_fwUserHumanoid
...
cvar_strong_dmg, cvar_axe_dmg, cvar_combat_dmg, cvar_katana_dmg, cvar_spawndelay, cvar_balrog_dmg,
...
cvar_hammer_radius2, cvar_zombie_distance, cvar_nata_distance, cvar_fire_knock,
g_ZBScore, g_HMScore, cvar_yaskha_speed, cvar_nvg_red, cvar_nvg_green, cvar_nvg_blue, cvar_alpha
...
// Models
new const v_model[] = "models/zm_cso/v_balrog9.mdl"
new const p_model[] = "models/zm_cso/p_balrog9.mdl"
...
new g_exp_sprid, g_exp_sprid2
new g_attack_mode[33], g_charging[33], g_charged[33], g_chargeattack[33]
...
public plugin_init() {
	...
	RegisterHam(Ham_Weapon_SendWeaponAnim, "weapon_knife", "fw_Knife_SendAnim", 1)
	RegisterHam(Ham_TraceAttack, "player", "fw_TraceAttackBA")
	RegisterHam(Ham_TraceAttack, "player", "fw_TraceAttack_PostBA", 1)

	register_forward(FM_EmitSound, "fw_EmitSoundHuman")
	register_forward(FM_EmitSound, "fw_EmitSoundBalrog")
	...
	register_forward(FM_CmdStart, "fw_CmdStartBalrog")
	...
	*/
	...
	cvar_pyscho_speed = register_cvar("cso_pyscho_speed", "260")
	cvar_china_force_speed = register_cvar("cso_china_force_speed", "400.0")
	...
	cvar_deimos_speed = register_cvar("cso_deimos_speed", "255")
	cvar_yaskha_speed = register_cvar("cso_yaksha_speed", "280")
	...
	// Holy Bomb
	pcvar_radius = register_cvar("cso_holy_radius", "150")
	pcvar_damage = register_cvar("cso_holy_damage", "75")
	pcvar_burn_damage = register_cvar("cso_holy_burn_damage","25")
	pcvar_burn_period = register_cvar("cso_holy_burns_period", "5")
	pcvar_burn_delay = register_cvar("cso_holy_burn_second", "0.5")
	
	// Jump Greande
	cvar_jump_radius = register_cvar("cso_frog_grenade_radius", "200")
	cvar_jump_knock = register_cvar("cso_frog_grenade_knockback", "350")
	
	//Fire Nades
	cvar_fire_knock = register_cvar("cso_fire_grenade_knockback", "100")
	
	// Knifes
	cvar_strong_dmg = register_cvar("cso_a_knife_strong_damage","5.0")
	cvar_axe_dmg = register_cvar("cso_a_knife_axe_damage", "4.0")
	cvar_combat_dmg = register_cvar("cso_a_knife_combat_damage", "4.75")
	cvar_katana_dmg = register_cvar("cso_a_knife_katana_damage", "3.75")
	cvar_balrog_dmg = register_cvar("cso_a_knife_balrog_damage", "3.5")
	
	cvar_hammer_dmg = register_cvar("cso_a_knife_hammer_dmg", "5.25")
	cvar_hammer_dmg2 = register_cvar("cso_a_knife_hammer_dmg2", "3.5")
	cvar_hammer_speed = register_cvar("cso_knife_hammer_speed", "180.0")
	cvar_hammer_speed2 = register_cvar("cso_knife_hammer_speed2", "150.0")
	cvar_hammer_radius = register_cvar("cso_knife_hammer_distance", "50.0")
	cvar_hammer_radius2 = register_cvar("cso_knife_hammer_distance2", "100.0")
	cvar_nata_distance = register_cvar("cso_knife_strong_distance", "30.0")

	cvar_zombie_distance = register_cvar("cso_w_zombie_distance", "20.0")
	
	cvar_human_speed = register_cvar("cso_human_speed", "230.0")
	cvar_multiratio = register_cvar("cso_multi_ratio", "0.15")
	
	cvar_nvg_red = register_cvar("zp_nvision_color_R", "190")
	cvar_nvg_green = register_cvar("zp_nvision_color_G", "90")
	cvar_nvg_blue = register_cvar("zp_nvision_color_B", "90")
	cvar_alpha = register_cvar("zp_nvision_alpha", "100")
	...
	register_concmd("cso_reset_user", "clcmd_reset_user", _, "<name>", 0)
	register_concmd("cso_make_zombie", "clcmd_make_zombie", _, "<name>", 0)
	register_concmd("cso_make_human", "clcmd_make_human", _, "<name>", 0)
	register_concmd("cso_user_respawn", "clcmd_user_respawn", _, "<name>", 0)
	register_concmd("cso_make_yaksha", "clcmd_make_yaksha", _, "<name>", 0)
	...
	g_fwUserInfected = CreateMultiForward("cso_user_infected", ET_IGNORE, FP_CELL)
	g_fwUserHumanoid = CreateMultiForward("cso_user_humanoid", ET_IGNORE, FP_CELL)

	//MYSQL
	g_pCvarDBInfo[Host] = register_cvar("cso_host", "127.0.0.1")
	g_pCvarDBInfo[User] = register_cvar("cso_user", "csoplayer")
	g_pCvarDBInfo[Pass] = register_cvar("cso_password", "odksqgisqgShittyCSO")
	g_pCvarDBInfo[DB] = register_cvar("cso_dbname", "levels_db")
...
public clcmd_get_user_stats(id)
{
	...
	for (Iterator = 1; Iterator <= get_maxplayers(); Iterator++)
	{
		if (is_user_connected(Iterator))
		{
			...
			{
				client_print ( id, print_console, "[ADMIN-CSO] Player %s / Level %d / EXP %d / Money %d /", s_Name, g_iLevel[Iterator], g_iExp[Iterator], g_CSOMoney[Iterator]);
			}
		}
	}
...
public clcmd_reset_user(id, level, cid)
{
	if(get_user_flags(id) & ADMIN_RCON) {
		
		new s_Name[ 32 ], s_Admin[ 32 ]
		...
		//g_levelpoints[i_Target] = 0
		g_iLevel[i_Target] = 0
		g_iExp[i_Target] = 0
		g_CSOMoney[i_Target] = 0
		/*
		g_levelswiftness[i_Target] = 0
		g_levelprowess[i_Target] = 0
		g_levelhealth[i_Target] = 0
		g_levelmarksmanship[i_Target] = 0
		g_levelprotection[i_Target] = 0
		*/
		client_print ( id, print_console, "[ADMIN-CSO] You have reset %s's stats.", s_Name)
		client_printc(i_Target, "!g[CSO]!n ADMIN %s has reset your stats.", s_Admin)
...
public clcmd_make_zombie(id, level, cid)
{
	if(get_user_flags(id) & ADMIN_RCON) {
		...
		new i_Target = cmd_target ( id, s_Name, (CMDTARGET_ONLY_ALIVE | CMDTARGET_ALLOW_SELF) );
		...
		if(g_zombie[i_Target])
		{
			client_print ( id, print_console, "[ADMIN-CSO] Player is already a zombie")
			return PLUGIN_HANDLED;
		}

		if(g_startround && !g_roundstart)
		{
			if(task_exists(1603))
				remove_task(1603)
			if(task_exists(1604))
			...
			if(task_exists(1501))
				remove_task(1501)
			infect_user(i_Target)
			fm_set_user_health(i_Target, 15000)
			check_teams()
			client_print(0, print_center, "A Zombie has appeared!")
			g_startround = false
			g_roundstart = true
			...
			infect_user(i_Target)
		client_print ( id, print_console, "[ADMIN-CSO] You have made %s a zombie.", s_Name)
		client_printc(i_Target, "!g[CSO]!n ADMIN %s made %s a zombie.", s_Admin, s_Name)

	}
	return PLUGIN_HANDLED;
}
public clcmd_make_human(id, level, cid)
{
	if(get_user_flags(id) & ADMIN_RCON) {
		...
		if(!g_zombie[i_Target])
		{
			client_print ( id, print_console, "[ADMIN-CSO] Player is already a human")
			return PLUGIN_HANDLED;
		}

		set_user_human(i_Target)
		client_print ( id, print_console, "[ADMIN-CSO] You have made %s a human.", s_Name)
		client_printc(i_Target, "!g[CSO]!n ADMIN %s made %s a human.", s_Admin, s_Name)

	}
	return PLUGIN_HANDLED;
}
public clcmd_user_respawn(id, level, cid)
{
	if(get_user_flags(id) & ADMIN_KICK) {
		...
		new i_Target = cmd_target ( id, s_Name, CMDTARGET_ALLOW_SELF );
		...
		{
			client_print ( id, print_console, "[ADMIN-CSO] Player is already a alive")
			return PLUGIN_HANDLED;
		}

		ExecuteHamB(Ham_CS_RoundRespawn, i_Target)
		client_print ( id, print_console, "[ADMIN-CSO] You have respawned %s.", s_Name)
		client_printc(i_Target, "!g[CSO]!n ADMIN %s respawned %s .", s_Admin, s_Name)
	}
	return PLUGIN_HANDLED;
}
public clcmd_make_yaksha(id, level, cid)
{
	if(get_user_flags(id) & ADMIN_RCON) {
		new s_Name[ 32 ], s_Admin[ 32 ]
		read_argv ( 1, s_Name, charsmax ( s_Name ) );
		...
		if(g_startround && !g_roundstart)
		{
			...
			if(task_exists(1501))
				remove_task(1501)
			g_nextclass[i_Target] = yaksha
			infect_user(i_Target)
			fm_set_user_health(i_Target, 20000)
			check_teams()
			client_print(0, print_center, "Yaksha has appeared!")
			client_printc(0, "!g[CSO]!n !tYaksha !nis here, do your best to get rid of him.")
			g_startround = false
			g_roundstart = true
			client_cmd(0,"mp3 play sound/zm_cso/ambience.mp3")
		}
...
public plugin_precache()
{
	...
	precache_generic( "sprites/knife_balrog9_bos92.txt" );
	precache_generic( "sprites/balrog9/640hud75.spr" )
	...
	register_clcmd("knife_balrog9_bos92", "HookKnife")
...
public MySQL_Init()
{
	...
	if (hSQLConnection != Empty_Handle)
	{
		...
		server_print("[MySQL][LVL] Successfully connected to host: %s (ALL IS OK).", szHost)
		SQL_FreeHandle(hSQLConnection)
	}
	else
	{
		// Disable plugin
		log_amx("[MySQL][LVL] An error has occured: %s", szError)
		server_print("[MySQL][LVL] An error has occured: %s", szError)
	}
...
public CPlayer__TakeDamage_Post(victim, inflictor, attacker, Float:damage)
{
	...
	if(is_user_valid_connected(inflictor) && get_user_weapon(inflictor) == CSW_KNIFE && !g_zombie[attacker] && g_zombie[victim])
	{
		...
		switch(g_iLevel[attacker])
		{
			case 0..9  : knockbackprowess = 0.0
			case 10..19 : knockbackprowess = 1.0
			case 20..29 : knockbackprowess = 1.5
			case 30..39 : knockbackprowess = 2.0
			case 40..49 : knockbackprowess = 2.5
			case 50..59 : knockbackprowess = 3.0
			case 60..69 : knockbackprowess = 3.5
			case 70..79 : knockbackprowess = 4.0
			case 80..89 : knockbackprowess = 4.5
			case 99..500 : knockbackprowess = 5.0
		}
...
public fw_EmitSoundZombies(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if (sample[7] == 'b' && sample[8] == 'h' && sample[9] == 'i' && sample[10] == 't')
	{
		...
		else if(g_playerclass[id] == yaksha)
		{
			...
			switch(Random)
			{
				case 0 : emit_sound(id, CHAN_AUTO, "zombi/akshazombie_hurt1.wav", 1.0, ATTN_NORM, 0, PITCH_NORM)
				case 1 : emit_sound(id, CHAN_AUTO, "zombi/akshazombie_hurt2.wav", 1.0, ATTN_NORM, 0, PITCH_NORM) 
			}
		}
		return FMRES_SUPERCEDE;
	}
...
public check_weapons_list(id)
{
	...
	else
	{
		...
		else if(g_knife[id] == balrog)
		{
			message_begin(MSG_ONE, gmsgWeaponList, {0,0,0}, id)
			write_string("knife_balrog9_bos92")
			write_byte(-1)
			write_byte(-1)
			write_byte(-1)
			write_byte(-1)
...
public fw_traceline(Float:vector_start[3], Float:vector_end[3], ignored_monster, id, handle)
{
	...
	switch(g_iLevel[id])
	{
		case 0..9 : distanceprowess = 0.0
		case 10..19 : distanceprowess = 1.0
		case 20..29 : distanceprowess = 1.5
		case 30..39 : distanceprowess = 2.0
		case 40..49 : distanceprowess = 2.5
		case 50..59 : distanceprowess = 3.0
		case 60..69 : distanceprowess = 3.5
		case 70..79 : distanceprowess = 4.0
		case 80..89 : distanceprowess = 4.5
		case 90..500 : distanceprowess = 5.0
	}
	if(g_buffed[id])
		distancebuff = 50.0
	else
...
public fw_EmitSoundBalrog(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if(get_user_weapon(id) != CSW_KNIFE || g_knife[id] != balrog)
		return FMRES_IGNORED
...
public fw_CmdStartBalrog(id, uc_handle, seed)
{
	if (!is_user_alive(id)) 
		return
	if(get_user_weapon(id) != CSW_KNIFE)
	...
	if(CurButton & IN_ATTACK)
	{
		...
		if(CurButton & IN_ATTACK2) 
		{
			...
			PlayEmitSound(id, weapon_sound[B9_CHARGE_START])
...
public Do_SetCharge(id)
{
	...
	PlayEmitSound(id, weapon_sound[B9_CHARGE_FINISH])
}
...
public fw_Knife_SendAnim(ent, anim, skip_local)
{
	...
	if(g_knife[id] == balrog && !g_zombie[id])
	{
...
public fw_TraceAttackBA(ent, attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	if(!is_user_alive(attacker))
		return HAM_IGNORED
	if(!g_zombie[attacker])
		return HAM_IGNORED
	if(get_user_weapon(attacker) != CSW_KNIFE || g_knife[attacker] != balrog)
		return HAM_IGNORED
		
	if(g_attack_mode[attacker] == MODE_NORMAL) SetHamParamFloat(3, Damage * DAMAGE_SLASH)
	else if(g_attack_mode[attacker] == MODE_CHARGE) SetHamParamFloat(3, Damage * DAMAGE_CHARGE)
	
	return HAM_HANDLED
}

public fw_TraceAttack_PostBA(ent, attacker, Float:Damage, Float:Dir[3], ptr, DamageType)
{
	if(!is_user_alive(attacker))
		return HAM_IGNORED
	if(!g_zombie[attacker])
...
public set_user_human(id)
{
	...
	switch(g_iLevel[id])
	{
		case 0..9 : bonushealth = 10
		case 10..19 : bonushealth = 20			
		case 20..29 : bonushealth = 30		
		case 30..39 : bonushealth = 40
		case 40..49 : bonushealth = 50
		case 50..59 : bonushealth = 55
		case 60..69 : bonushealth = 60
		case 70..79 : bonushealth = 65
		case 80..89 : bonushealth = 75
		case 90..500 : bonushealth = 80
		default : bonushealth = 0
	}
	if(get_user_flags(id) & ADMIN_LEVEL_H)
	{
		fm_set_user_health(id, 150 + bonushealth)
	}
	else
	{
	...
	remove_task(785485+id)
	ExecuteForward(g_fwUserHumanoid, g_fwDummyResult, id)
}
...
public Print_VIP_List()
{
	...
	{
		if (!is_user_valid_connected(iPlayer) || !(get_user_flags(iPlayer) & ADMIN_LEVEL_H))
			continue
	...
	for (iPlayer = 1; iPlayer <= g_iMaxClients; iPlayer++)
	{
		...
		client_printc(iPlayer, "!g[CSO]!n %s", szMessage)
		get_pcvar_string(g_pCvarAdminContact, szContact, charsmax(szContact))

		if (szContact[iPlayer])
		{
			client_printc(iPlayer, "!g[CSO]!n Contact server administrator : !t%s", szContact)
		}
	}
}
...
{
	if(g_zombie[id] && is_user_valid_connected(id) && g_playerclass[id] != yaksha)
	{
		if(is_user_alive(id))
		{
			if(!g_using_ab[id])
...
public client_PreThink(id)
{
	...
	switch(g_iLevel[id])
	{
		case 0..9 : speedswiftness = 1
		case 10..19 : speedswiftness = 2
		case 20..29 : speedswiftness = 3
		case 30..39 : speedswiftness = 4
		case 40..49 : speedswiftness = 5
		case 50..59 : speedswiftness = 6
		case 60..69 : speedswiftness = 7
		case 70..79 : speedswiftness = 8
		case 80..89 : speedswiftness = 9
		case 90..500 : speedswiftness = 10
		default : speedswiftness = 0
	}
	if(g_zombie[id])
	{
		if(g_playerclass[id] == light)
...
public event_TeamInfo()
{
	...
	if(fnGetHumans() > 1)
	{
		if(!task_exists(id+TASK_SPAWN) && !(round_count == 5 || round_count == 10))
		{
...
public fw_ClientDisconnect_Post(id)
{
	...
	if(g_zombie[id])
	{
		if(!fnGetZombies() && iPlayersnum > 2)
		{
			...
			if(g_playerclass[id] == yaksha)
			{
				g_nextclass[newid] = yaksha
				infect_user(newid)
			}
			else
				infect_user(newid)
...
public event_round_start()
{
	...
	if(round_count == 5 || round_count == 10)
		client_printc(0, "!g[CSO]!n This is a special round, !tYaksha will appear and you will have to eliminate him.")
	/*
	if(round_count >= 1)
	{*/
	if(task_exists(1603))
...
public make_countdown() {
	...
	for(new i = 0; i < g_iMaxClients; i++)
	{
		...
		if(countdown_timer <= 10)
		{
			...
			client_print(i, print_center, "Game starts in %d seconds", countdown_timer)
		}
	}
...
public start_game(player_id)
{
	...
	if(round_count == 5 || round_count == 10 && iPlayersnum >= 2)
	{
		g_nextclass[id] = yaksha
		infect_user(id)
		fm_set_user_health(id, 20000)
		check_teams()
		client_print(0, print_center, "Yaksha has appeared!")
		client_printc(0, "!g[CSO]!n !tYaksha !nis here, do your best to get rid of him.")
		g_startround = false
	...
	{	
		...
		check_teams()
		client_print(0, print_center, "Zombies has appeared!")
		g_startround = false
	}
...
public fw_TakeDamage(victim, inflictor, attacker, Float:damage, damage_type) 
{
	...
	switch(g_iLevel[victim])
	{
		case 0..9 : damage *= 1.0
		case 10..19 : damage *= 0.95
		case 20..29 : damage *= 0.90
		case 30..39 : damage *= 0.85
		case 40..49 : damage *= 0.80
		case 50..59 : damage *= 0.75
		case 60..69 : damage *= 0.70
		case 70..79 : damage *= 0.65
		case 80..89 : damage *= 0.60
		case 90..500 : damage *= 0.50
		default : damage *= 1.0
	}
	new Float:damageprowess
	switch(g_iLevel[attacker])
	{
		case 0..9 : damageprowess = 1.1
		case 10..19 : damageprowess = 1.2
		case 20..29 : damageprowess = 1.3
		case 30..39 : damageprowess = 1.4
		case 40..49 : damageprowess = 1.5
		case 50..59 : damageprowess = 1.6
		case 60..69 : damageprowess = 1.7
		case 70..79 : damageprowess = 1.8
		case 80..89 : damageprowess = 1.9
		case 90..100 : damageprowess = 2.0
		default : damageprowess = 1.0
	}
	new Float:damagemarks
	switch(g_iLevel[attacker])
	{
		case 0..9 : damagemarks = 1.0
		case 10..19 : damagemarks = 1.05
		case 20..29 : damagemarks = 1.10
		case 30..39 : damagemarks = 1.15
		case 40..49 : damagemarks = 1.20
		case 50..59 : damagemarks = 1.25
		case 60..69 : damagemarks = 1.30
		case 70..79 : damagemarks = 1.35
		case 80..89 : damagemarks = 1.40
		case 90..500 : damagemarks = 1.50
	...
	if(g_zombie[attacker] && !g_zombie[victim])
	{
		if(fnGetHumans() != 1  && !(round_count == 5 || round_count == 10))
		{
			static Float:armor
			...
			if (armor > 0.0)
			{
				...
				client_print(attacker, print_center, "Armor : %d", get_user_health(victim) - floatround(damage))
				return HAM_SUPERCEDE;
			}
		...
		else
		{
			...
			if(g_playerclass[attacker] == yaksha)
	...
	else if(!g_zombie[attacker] && g_zombie[victim])
	{
		if (is_user_valid_connected(inflictor) && get_user_weapon(inflictor) == CSW_KNIFE)
		{
			...
			else if(g_knife[attacker] == balrog)
				SetHamParamFloat(4, damage * get_pcvar_float(cvar_balrog_dmg) * damageprowess);
		}
...
public fw_TraceAttack(victim, attacker, Float:damage, Float:direction[3], tracehandle, damage_type) {
	...
	if (g_zombie[victim])
	{
		if(g_playerclass[victim] == yaksha)
			xs_vec_mul_scalar(direction, get_pcvar_float(cvar_zmknockback) / 5, direction)
		else if(g_playerclass[victim] == deimos)
			xs_vec_mul_scalar(direction, get_pcvar_float(cvar_zmknockback) / 2, direction)
		else
...
public ShowHUD(taskid)
{
	...
	if(round_count == 5 || round_count == 10)
	{
		...
		show_dhudmessage(id, "[%i] ZB ( %i ) HM [%i]^nROUND^nEliminate Yaksha", g_ZBScore, round_count, g_HMScore);	
	}
	/*
	else if (round_count > 10)
	{
		set_dhudmessage(random(255), random(255), random(255), -1.0, 0.0, 0, 6.0, 1.1, 0.0, 0.0);
		show_dhudmessage(id, "[%i] ZB ( CHANGING MAP ) HM [%i]^nROUND^nSTATUS : %s^nTIME TO CHOOSE : %i", g_ZBScore, g_HMScore, Voted[id] ? "Voted" : "Not Voted", Timer);	
	}
	*/
	else
	{
		set_dhudmessage(random(255), random(255), random(255), -1.0, 0.0, 0, 6.0, 1.1, 0.0, 0.0);
		show_dhudmessage(id, "[%i] ZB ( %i ) HM [%i]^nROUND", g_ZBScore, round_count, g_HMScore);	
	}
	if(g_zombie[id])
	{
		set_dhudmessage(random(255), random(255), random(255), -1.0, 0.9, 0, 6.0, 1.0)
		if(g_playerclass[id] == yaksha)
			show_dhudmessage(id, "Press G for %s || T for Health", szAB)
		else
			show_dhudmessage(id, "Press G for %s", szAB)
	}
	set_dhudmessage(255, 0, 0, 0.02, 0.18, 0, 6.0, 1.0)
	show_dhudmessage(id, "Health: %d^nLevels: %d/100^nXP: %d/%d^nClass : %s", get_user_health(id), g_iLevel[id], g_iExp[id], MAX_LEVELS[g_iLevel[ id ]], szClass)
}
/* Menu */
...
public show_game_menu(id) 
{
	...
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r3\y].\w Skin Management \r(soon)^n")
	if(get_user_flags(id) & ADMIN_KICK)
...
public show_class_menu(id) {
	...
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r2\y].\w Regular \d: \rSmoke^n")
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r3\y].\w Jiang Shi \d: \rSpeed^n")
	len += formatex(menu[len], charsmax(menu) - len, "\y[\r4\y].\w Voodoo \d: \rHealing^n")
	if(get_user_flags(id) & ADMIN_LEVEL_H)
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r5\y].\w Deimos \d: \rTesla\y [\rVIP\y]")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\y[\r5\y].\d Deimos \d: \rTesla\y [\rVIP\y]")
...
public menu_class(id, key) {
	switch (key)
	{
	    case 0:
		{
			...
			client_printc(id, "!g[CSO]!n Your next class will be : !tLight!n!")
		}
	         case 1:
		{
			g_nextclass[id] = normal
			client_printc(id, "!g[CSO]!n Your next class will be : !tRegular!n!")
		}
		case 2:
		{
			g_nextclass[id] = china
			client_printc(id, "!g[CSO]!n Your next class will be : !tJiang Shi!n!")
		}
		case 3:
		{
			g_nextclass[id] = voodoo
			client_printc(id, "!g[CSO]!n Your next class will be : !tVoodoo!n!")
		}
		case 4:
		{
			if(get_user_flags(id) & ADMIN_LEVEL_H)
			{
				g_nextclass[id] = deimos
				client_printc(id, "!g[CSO]!n Your next class will be : !tDeimos!n!")
			}
		}
	}
	return PLUGIN_HANDLED;
...
public show_knife_menu(id) {
	...
	if(g_iLevel[id] > 9)
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\w Katana \rLVL 10^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r4.\d Katana \rLVL 10^n")

	if(g_iLevel[id] > 19)
		len += formatex(menu[len], charsmax(menu) - len, "\r5.\w Hammer \rLVL 20^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r5.\d Hammer \rLVL 20^n")

	if(get_user_flags(id) & ADMIN_LEVEL_H)
		len += formatex(menu[len], charsmax(menu) - len, "\r6.\w Balrog 9 \rVIP^n")
	else
		len += formatex(menu[len], charsmax(menu) - len, "\r6.\d Balrog 9 \rVIP^n")
...
public infect_user(id)
{
	...
	switch(g_iLevel[id])
	{
		case 0..9 : bonushealth = 100
		case 10..19 : bonushealth = 200		
		case 20..29 : bonushealth = 300		
		case 30..39 : bonushealth = 400
		case 40..49 : bonushealth = 500
		case 50..59 : bonushealth = 600
		case 60..69 : bonushealth = 700
		case 70..79 : bonushealth = 800
		case 80..89 : bonushealth = 900
		case 90..500 : bonushealth = 1000
		default : bonushealth = 0
	}
	...
	ExecuteForward(g_fwUserInfected, g_fwDummyResult, id)
}
...
public Save_Data(id)
{
	...
	*/
	...
	new szQuery[1024]
	formatex(szQuery, charsmax(szQuery), "REPLACE INTO `my_table` (`Level`, `XP`, `KNIFE`, `SKIN`, `MONEY`, `NAME`, `SteamID`) VALUES ('%d', '%d', '%d', '%d', '%d', '%s', '%s');", g_iLevel[id], g_iExp[id], g_knife_key[id], skin[id], g_CSOMoney[id], szName, szAuthID)
...
public Load_Data(id)
{
	...
	*/
	...
	formatex(szQuery, charsmax(szQuery), "SELECT * FROM `my_table` WHERE ( `SteamID` = '%s' );", szAuthID)
...
public QuerySelectData(iFailState, Handle:hQuery, szError[], iError, szData[]) 
{
	...
	if (!SQL_NumResults(hQuery))
	{
		...
		/*
		g_levelprowess[id] = 0
		g_levelprotection[id] = 0
		g_levelswiftness[id] = 0
		g_levelmarksmanship[id] = 0
		g_levelhealth[id] = 0
		g_levelpoints[id] = 0
		*/
		return
	...
	g_iExp[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "XP"))
	skin[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "SKIN"))
	g_knife_key[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "KNIFE"))
	g_CSOMoney[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "MONEY"))
	/*
	g_levelprowess[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "PROWESS"))
	g_levelprotection[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "PROTECTION"))
	g_levelswiftness[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "SPEED"))
	g_levelmarksmanship[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "MARKSMANSHIP"))
	g_levelhealth[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "HEALTH"))
	g_levelpoints[id] = SQL_ReadResult(hQuery, SQL_FieldNameToNum(hQuery, "POINTS"))*/
}
public Cmd_NvgToggle(id)
{
	if (is_user_alive(id))
	{
		if(g_zombie[id])
		{
			...
			{
				g_bNvgOn[id] = true
				set_task(0.1, "set_nightvision", 785485+id, _, _, "b")
				Set_MapLightStyle(id, "z")
			}
			else
			{
...
{
	if(!is_user_valid_connected(id) || !g_zombie[id] || !g_bNvgOn[id])
		return;
	...
	write_short(1<<10)
	write_short(0x000)
	write_byte(get_pcvar_num(cvar_nvg_red)) // r
	write_byte(get_pcvar_num(cvar_nvg_green)) // g
	write_byte(get_pcvar_num(cvar_nvg_blue)) // b
	write_byte(get_pcvar_num(cvar_alpha))
	message_end()
}
fnGetPlaying()
{
...
public ActionSpecial()
{
	...
	for( new i = 1; i <= g_iMaxClients; i++ ) //show menu
	{
		if(is_user_valid_connected(i))
		{
			Voted[i] = false;
			ChangeMaps(i)
			client_printc(i, "!g[CSO]!n Map voting has begun.")
			message_begin(MSG_ONE, g_msgScreenFade, {0, 0, 0}, i)
			write_short(floatround(4096.0 * 20.0, floatround_round))
			write_short(floatround(4096.0 * 20.0, floatround_round))
			write_short(4096)
			write_byte(0)
			write_byte(0)
...
public ChangeMaps(client)
{
	static szMap[128]; 
	new st[ 3 ];
	formatex(szMap, charsmax(szMap)-1, "\w Choose Map:^n\r// \wStatus: %s^n\r// \wTime Remaining: \y%i",Voted[client] ? "\yVoted" : "\rNot Voted", Timer); 
...
{ 
	...
	client_printc( 0, "!g[CSO]!n Map !t%s !nwon the vote with !t%i !npicks.", g_maps[ Winner ], g_szKind[ Winner ] );
	new map[30]
...
stock bool:SQL_IsFail( const iFailState, const iError, const szError[ ] )
{
	...
	{
		log_amx( "[SQL] Could not connect to SQL database: %s", szError );
		return true;
	}
	else if( iFailState == TQUERY_QUERY_FAILED )
	{
		log_amx( "[SQL] Query failed: (%i) %s", iError, szError );
		return true;
	}
